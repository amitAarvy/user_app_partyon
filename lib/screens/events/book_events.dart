...
class _BookEventsState extends State<BookEvents> {
  ...
  getEventOrganiserID() async {
    ...
    try {
      ...
      final DocumentSnapshot documentSnapshot = await FirebaseFirestore.instance.collection('Events').doc(widget.eventID).get();
      ...
      if (eventOrganiserID != '') {
        DocumentSnapshot<Map<String, dynamic>> data = await FirebaseFirestore.instance
            ...
            .get();
        organiser = data.data();
        QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection('Follow').where('uid', isEqualTo: eventOrganiserID).get();
      ...
      await FirebaseFirestore.instance.collection("Club").doc(widget.clubUID).get().then((doc) async {
        if (doc.exists) {
          address = '${getKeyValueFirestore(
                doc,
                'address',
      ...
      if (widget.isVenue == true) {
        ...
        List data1 = data.docs.where((element) => element['prId'].toString() == widget.promoterID).where((e) => e['eventId'].toString() == widget.eventID.toString()).toList();
      ...
      appBar: AppBar(
        ...
        actions: [
          GestureDetector(
            onTap: () async {
              String imageURL = '';
              await FirebaseFirestore.instance.collection('Club').doc(widget.clubUID).get().then((DocumentSnapshot<Map<String, dynamic>> value) => imageURL = value.data()?['layoutImage'] ?? '');
                      ...
                      return Column(
                        children: [
                                ...
                                Card(
                                  ...
                                  child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Text("Event info ", style: GoogleFonts.adamina(fontSize: 22, fontWeight: FontWeight.w700, color: Colors.white, decoration: TextDecoration.underline, decorationColor: Colors.white, decorationThickness: 1)),
                                          ...
                                          Row(
                                            children: [
                                              ...
                                              const SizedBox(width: 5),
                                              Text(" ${endTime.difference(startTime).inHours} hours", style: GoogleFonts.adamina(fontSize: 15, color: Colors.white)),
                                            ],
                                          ),
                                          const SizedBox(height: 10),
                                          // Row(
                          ...
                          Padding(
                              ...
                              child: Align(
                                alignment: Alignment.centerLeft,
                                child: Text("Venue", style: GoogleFonts.adamina(fontSize: 22, fontWeight: FontWeight.w700, color: Colors.white, decorationColor: Colors.white)),
                              )),
                          SizedBox(
                            height: 32.h,
                          ),
                          Padding(
                              ...
                              child: Row(
                                children: [
                                  Icon(Icons.door_back_door_outlined, color: Colors.white, size: 52.h),
                                  const SizedBox(width: 5),
                                  Text("Doors open at ${TimeOfDay(hour: startTime.hour, minute: startTime.minute).hourOfPeriod.toString()}:${TimeOfDay(hour: startTime.hour, minute: startTime.minute).minute.toString().padLeft(2, '0')} ${TimeOfDay(hour: startTime.hour, minute: startTime.minute).period.name}",
                                      style: GoogleFonts.adamina(fontSize: 15, color: Colors.white)),
                                ],
                              )),
                          SizedBox(height: 10),
                          Padding(
                              ...
                              child: Row(
                                children: [
                                  Icon(Icons.location_on, color: Colors.white, size: 52.h),
                                  const SizedBox(width: 5),
                              ...
                              child: GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (context) => OrganiserEvent(
                                                clubName: organiser!['companyMame'].toString(),
                                                clubUid: getKeyValueFirestore(data, 'organiserID'),
                                    ...
                                    child: Column(
                                      children: [
                                        ...
                                        AspectRatio(
                                          aspectRatio: 16 / 9,
                                          child: Image.network(
                                            organiser!['profile_image'].toString(),
                                            fit: BoxFit.cover,
                                          ),
                                        ),
                                        SizedBox(height: 10),
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              organiser!['companyMame'].toString(),
                                        ...
                                        ValueListenableBuilder(
                                          ...
                                          builder: (context, bool isCheck, child) => Row(
                                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                            children: [
                                              Text(
                                                'Become a Follower\n10 Followers',
                          ...
                          SizedBox(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                ValueListenableBuilder(
                                    valueListenable: applyCoupon,
                                    builder: (context, bool value, child) => Consumer2<EntryTableController, EntryController>(
                                                        ...
                                                        if (totalEntranceCount > 0 || totalTableCount > 0) {
                                                          if (getKeyValueFirestore(data, 'entryManagementCouponList').toString() != 'null' || getKeyValueFirestore(data, 'entryManagementCouponList').toString() != 'null') {
                                                            ...
                                                            if (getKeyValueFirestore(data, 'entryManagementCouponList')['couponCode'].toString() == enteredCode) {
                                                              print('check 2');
                                                              couponDetail.value = {"appliedCoupon": 'venue', 'eventDetail': data, 'eventId': widget.eventID, 'totalView': 0, "data": getKeyValueFirestore(data, 'entryManagementCouponList'), "type": 'entry', 'coupon': getKeyValueFirestore(data, 'entryManagementCouponList')['tableCoupon']};
                                                              ...
                                                              print('check 3');
                                                              couponDetail.value = {"appliedCoupon": 'venue', 'eventId': widget.eventID, "data": getKeyValueFirestore(data, 'tableManagementCouponList'), 'totalView': 0, "type": 'table', 'coupon': getKeyValueFirestore(data, 'tableManagementCouponList')['tableCoupon']};
                                                              applyCoupon.value = true;
                                                              print('Coupon matched: $couponDetail');
                                                              double couponDiscount = 0;
                                                              if (couponDetail.value != null && couponDetail.value!['data']['discount'] != null) {
                                                              ...
                                                              if (eventCoupons.isNotEmpty) {
                                                                ...
                                                                for (var doc in eventCoupons) {
                                                                  final entryCode = doc['entryCoupon'].toString() == 'null' ? null : doc['entryCoupon']['couponCode'].toString();
                                                                  final tableCode = doc['tableCoupon'].toString() == 'null' ? null : doc['tableCoupon']['couponCode'].toString();

                                                                  print('check table coupon code $tableCode');
                                                                  if (entryCode != null) {
                                                                    if (totalEntranceCount > 0 && enteredCode == entryCode) {
                                                                      promoterId.value = doc['prId'].toString();
                                                                      matchedCouponCode = entryCode;
                                                                      couponDetail.value = {"appliedCoupon": doc['isInf'] == true ? 'inf' : 'pr', "totalView": doc['totalUseCoupon'], "id": doc.id, "type": 'entry', 'eventId': widget.eventID, 'data': doc['entryCoupon'], 'coupon': doc['entryCoupon']['couponCode']};
                                                                      matchFound = true;
                                                                      break;
                                                                    }
                                                                  }
                                                                ...
                                                                if (matchFound) {
                                                                  ...
                                                                  Fluttertoast.showToast(msg: 'No valid coupon code found for this event.');
                                                                }
                                                              } else {
                                                                Fluttertoast.showToast(msg: 'No coupon code found for this event.');
                                                              }
                                                            }
                                                          } else {
                                                            print('check 5');
                                                              ...
                                                              for (var doc in eventCoupons) {
                                                                ...
                                                                if (entryCode != null) {
                                                                  if (totalEntranceCount > 0 && enteredCode == entryCode) {
                                                                    promoterId.value = doc['prId'].toString();
                                                                    matchedCouponCode = entryCode;
                                                                    couponDetail.value = {"appliedCoupon": "pr", "totalView": doc['totalUseCoupon'], "id": doc.id, "type": 'entry', 'data': doc['entryCoupon']};
                                                                    matchFound = true;
                                                                    break;
                                                                  }
                                                                }
                                                                ...
                                                                if (tableCode != null) {
                                                                  if (totalTableCount > 0 && enteredCode == tableCode) {
                                                                    promoterId.value = doc['prId'].toString();
                                                                    matchedCouponCode = tableCode;
                                                                    couponDetail.value = {"totalView": doc['totalUseCouponTable'], "id": doc.id, "type": 'table', 'data': doc['tableCoupon']};
                                                                    matchFound = true;
                                                                    break;
                                                                  }
                                                                }
                                                          ...
                                                          Fluttertoast.showToast(msg: 'Please add entry or table management.');
                                                        }
                  ...
                  Container(
                    ...
                    child: Center(
                      child: ValueListenableBuilder(
                        ...
                        builder: (context, prId, child) => ValueListenableBuilder(
                          valueListenable: applyCoupon,
                          builder: (context, bool isApply, child) => ValueListenableBuilder(
                            valueListenable: couponDetail,
                            builder: (context, Map<String, dynamic>? value, child) => Consumer<EntryTableController>(
                                        ...
                                        if (uid() != null) {
                                          ...
                                          if (tablePrice > 0) {
                                            ...
                                            for (int i = 0; i < data.tableName.length; i++) {
                                              ...
                                              tableData[i] = {'tableID': randomAlphaNumeric(5).toUpperCase(), 'tableName': data.tableName[i], 'tableNum': data.numTable[i], 'tableLeft': data.numTable[i] ?? 0 - data.leftBooking[i] ?? 0, 'tablePrice': data.priceTable[i]};
                                            }
                                          ...
                                          } else if (bookingProvider.getBookingList.isNotEmpty || bookingProvider.getTableBookingList.isNotEmpty) {
                                            if (!context.mounted) return;
                                            paymentSuccess(context, null,
                                                tableDataList: tableData.where((element) => element['tableName'].toString() != '').toList(),
                                                amount: amount,
