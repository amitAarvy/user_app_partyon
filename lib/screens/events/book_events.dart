...
class _BookEventsState extends State<BookEvents> {
  ...
  void showModalForWeb(BuildContext context) async {
    final data = await FirebaseFirestore.instance.collection('globalSettings').get();
    final showCloseIcon = data.docs.isNotEmpty && data.docs[0]['popIcon'].toString() == 'true';
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                image: DecorationImage(
                  image: AssetImage('assets/webBackground.png'),
                  fit: BoxFit.fill,
                ),
              ),
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      GestureDetector(
                        onTap: () => _launchUrl('https://play.google.com/store/apps/details?id=com.partyon.user'),
                        child: Image.asset(
                          'assets/img.png',
                          height: 0.1.sh,
                          width: 0.35.sw,
                      ...
                      GestureDetector(
                        onTap: () => _launchUrl('https://apps.apple.com/us/app/hashtag-partyon/id6738373315'),
                        child: Image.asset(
                          'assets/img_1.png',
                          height: 0.1.sh,
                          width: 0.35.sw,
                                                   ...
                                                   if (totalEntranceCount > 0 || totalTableCount > 0) {
                                                     ...
                                                     if(getKeyValueFirestore(data, 'entryManagementCouponList').toString() != 'null' || getKeyValueFirestore(data, 'entryManagementCouponList').toString() != 'null'){
                                                       ...
                                                       if(getKeyValueFirestore(data, 'entryManagementCouponList')['couponCode'].toString() == enteredCode){
                                                        ...
                                                        getKeyValueFirestore(data, 'tableManagementCouponList') != null &&
                                                           getKeyValueFirestore(data, 'tableManagementCouponList')['couponCode'].toString() == enteredCode){
                                                         print('check 3');
                                                         couponDetail.value = {
                                                           "appliedCoupon":'venue',
                                                           'eventId':widget.eventID,
                                                         ...
                                                         if (eventCoupons.isNotEmpty) {
                                                             ...
                                                             if (entryCode != null) {
                                                               if (totalEntranceCount > 0) {
                                                                 ...
                                                                 if(doc['filler'].toString() != '' &&   enteredCode.toLowerCase() == doc['filler'].toString().toLowerCase()){
                                                                   ...
                                                                   var allFillerBooking =  await FirebaseFirestore.instance.collection('FillerEventBooking').doc(widget.eventID).get();
                                                                   List filterData = (prFiller.docs as List).where((element) => element['eventId'].toString() ==widget.eventID.toString()).where((ele) => ele['prId'].toString() == doc['prId'].toString()).toList();
                                                                   var mapData = filterData[0].data() as Map<String,dynamic>;
                                                                   var fillerBookingList = allFillerBooking.data() == null?null:allFillerBooking.data() as Map<String,dynamic>;
                                                                   bool checkUserList = mapData.containsKey('userList');

                                                                   if(fillerBookingList != null && (fillerBookingList['dataList'] as List).length >= int.parse(doc['data']['pomotionData']['noOfEntry'].toString())){
                                                                     Fluttertoast.showToast(msg: 'Filler coupon code usage limit reached');
                                                                     return;
                                                                   }


                                                                   if(checkUserList){
                                                                     List userBookingList = (mapData['userList'] as List).where((element) => element['userId'].toString() == uid()).where((ele) => ele['couponDetail']['appliedCoupon'].toString() =='filler').toList();
                                                                     if(userBookingList.isNotEmpty){
                                                                       Fluttertoast.showToast(msg: 'Only one filler coupon code can be used per booking');
                                                                       return;
                                                                     }
                                                                   }

                                                                   if(totalEntranceCount >2){
                                                                     Fluttertoast.showToast(msg: 'Only two bookings are allowed per user.');
                                                                     return;
                                                                   }
                                                                   promoterId.value = doc['prId'].toString();
                                                                   matchedCouponCode = entryCode;
                                                                   couponDetail.value = {
                                                                     "appliedCoupon":'filler',
                                                           ...
                                                           if (matchFound) {
                                                             ...
                                                             print('Coupon matched: $couponDetail');
                                                             double couponDiscount =0;
                                                             if (couponDetail.value != null &&couponDetail.value!['appliedCoupon'] != 'filler' && couponDetail.value!['data']['discount'] != null) {
                                                               couponDiscount = double.parse(couponDetail.value!['data']['discount'].toString()) / 100;
                                                             }
                                                             if (couponDetail.value != null &&couponDetail.value!['appliedCoupon'] == 'filler' ) {
                                                               couponDiscount = 100 / 100;
                                                             }
                             ...
                             children: [
                               ValueListenableBuilder(
                                 ...
                                 builder: (context, Map<String,dynamic>? data, child) =>
                                     Text(
                                       'Applied Discount: ${data==null ?'0':data['appliedCoupon'].toString() == 'filler'?'100':data['data']['discount']}',
                                                ...
                                                return GestureDetector(
                                                  onTap: () async {
                                                    ...
                                                    if (uid() != null) {
                                                      ...
                                                      final double amount = tablePrice + entryPrice;
                                                      double couponDiscount = 0;
                                                      if (value != null && value['appliedCoupon'].toString() !='filler' && value['data']['discount'] != null) {
                                                        couponDiscount = double.parse(value['data']['discount'].toString()) / 100;
                                                      }
                                                      if (value != null && value['appliedCoupon'].toString() =='filler') {
                                                        couponDiscount = 100/ 100;
                                                      }
                                                      final double discountAmount =amount * couponDiscount ;
                                                      ...
                                                      if (couponCodeAmount > 0) {
                                                        ...
                                                        // paymentSuccess(context, null,
                                                        //     tableDataList: tableData.where((element) => element['tableName'].toString() !='').toList(),
                                                        //     amount: totalAmount == null?amount:totalAmount,
                                                        //     couponDetail: value,
                                                        //     clubUID: widget.clubUID,
                                                  ...
                                                  child: Padding(
                                                    ...
                                                    child: Column(
                                                      ...
                                                      children: [
                                                        Text(
                                                          '₹ ${totalAmount == null?tablePrice + entryPrice:totalAmount}',
                                                          style: GoogleFonts.ubuntu(
                                                            color: Colors.white,
                                                            fontSize:45.sp,
                                                          ),
